...
# data = np.load(file="bci_rgb_eeg.npy")
# label = np.load(file="bci_labels.npy")
# label[label < 0] = 0
...
# train_X = data[100:]
# train_y = label[100:]
train_X = np.load(file="bci_rgb_eeg_train.npy")
train_y = np.load(file="bci_labels_train.npy")
train_y[train_y < 0] = 0

test_X = np.load(file="bci_rgb_eeg_test.npy")
test_y = np.load(file="bci_labels_test.npy")
test_y[test_y < 0] = 0
...
#### main parameters ####
#########################
parser.add_argument("--dump_path", type=str, default="./eval",
...
def main():
    ...
    # build data
    train_dataset = BCIDataset(train_X,train_y,transform_train)
    val_dataset = BCIDataset(test_X,test_y,transform_test)
    ...
    linear_classifier = RegLog(2, args.arch, args.global_pooling, args.use_bn)
